...
import { updateProfile } from '@/features/settings/account/accountActions';
import { makeUpdateEntityOptions } from '@/features/settings/account/accountHelpers';
import { ProfileState } from '@/features/settings/account/accountSlice';
...
export const ProfileCard: FC = () => {
	...
	const [profileUpdate, setProfileUpdate] = useState<ProfileState | null>(account.profile);

	const handleCancelClicked = () => setProfileUpdate(account.profile);

	const handleFieldChanged = (field: keyof ProfileState) => (e: ChangeEvent<HTMLInputElement>) => {
		const { value } = e.target as HTMLInputElement;
		setProfileUpdate((prev) => ({ ...prev, [field]: value.trim() }));
	};

	const handleFailedToUpdate = () => snackbar.error('Failed to update profile');

	const handleSaveClicked = async () => {
		try {
			const payload = { ...profileUpdate, id: account.profile.id };
			const response = await dispatch(updateProfile(makeUpdateEntityOptions(payload)));
